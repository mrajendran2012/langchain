...
def call_model(state: State):
    ...
    input_dict = {"messages": trimmed_messages, "language": state["language"]}
    
    chain = prompt_template | llm | parser
    response = chain.invoke(input_dict)
    # Wrap the response string in an AIMessage. But it ends up printing twice, so we return it as a string.
    #return {"messages": trimmed_messages + [AIMessage(content=response)], "language": state["language"]}
